...
class ResNet_s(nn.Module):
    ...
    def __init__(self, block, num_blocks, num_classes=10, use_norm=False):
        ...
        self.in_planes = 32  # ← layer2 の出力チャンネルに合わせる！
        #self.att_layer3 = self._make_layer(block, 64, num_blocks[2], stride=1)
        #self.att_layer3 = self._make_layer(block, 64, num_blocks[2], stride=1, option='B')
        self.bn_att = nn.BatchNorm2d(64)
        self.att_conv = nn.Conv2d(64, num_classes, kernel_size=1, padding=0, bias=False)
    ...
    def forward(self, x):
        ...
        # attention branch
        ax = F.relu(self.bn_att2(self.att_conv(out)))
        ...
        rx = out * self.att
        rx = rx + out
        rx = self.layer4(rx)  # 8x8
        feature = F.avg_pool2d(rx, rx.size()[3])
        feature = feature.view(feature.size(0), -1)
        out = self.linear(feature)
        return out, ax, self.att, feature
...
def resnet32_abn(num_classes=10, use_norm=False):
